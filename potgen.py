...
class q:
    ...
    def equivalencemwcoords(
        ...
        vals = []
        mini, maxi, minj, maxj = (
            int(round(0.5 * self.numpoints)),
            min(self.numpoints * 3, 255),
            int(round(0.5 * q_other.numpoints)),
            min(q_other.numpoints * 3, 255),
        )
        if uselowest:
        ...
        if self.coordtype == 0 and q_other.coordtype == 0 and not forcegrid:
            ...
            if innerbound:
                minbnds = (
                    multiply(self.maxval, 0.8),
                    multiply(self.minval, 1.000000000),
                    multiply(q_other.maxval, 0.8),
                    multiply(q_other.minval, 1.000000000),
                )
                maxbnds = (
                    multiply(self.maxval, 1.0),
                    multiply(self.minval, 1.3),
                    multiply(q_other.maxval, 1.0),
                    multiply(q_other.minval, 1.3),
                )
            else:
                minbnds = (
                    multiply(self.maxval, 0.8),
                    multiply(self.minval, 0.8),
                    multiply(q_other.maxval, 0.8),
                    multiply(q_other.minval, 0.8),
                )
                maxbnds = (
                    multiply(self.maxval, 1.3),
                    multiply(self.minval, 1.3),
                    multiply(q_other.maxval, 1.3),
                    multiply(q_other.minval, 1.3),
                )
            ...
            if innerbound:
                minbnds = (
                    multiply(self.maxval, 0.9),
                    multiply(self.minval, 1.000000001),
                )
                maxbnds = (
                    multiply(self.maxval, 0.9999999999),
                    multiply(self.minval, 1.1),
                )
            else:
            ...
            for i in range(self.numpoints, mini, -1):
                for j in range(q_other.numpoints, minj, -1):
                    result = minimize(
                        frang, c, args=(m1, m2, q2range, i, j), bounds=bnds
                    )
            ...
            if innerbound:
                minbnds = (
                    multiply(q_other.maxval, 0.9),
                    multiply(q_other.minval, 1.000000001),
                )
                maxbnds = (
                    multiply(q_other.maxval, 0.9999999999),
                    multiply(q_other.minval, 1.1),
                )
            else:
            ...
            for i in range(self.numpoints, mini, -1):
                for j in range(q_other.numpoints, minj, -1):
                    ...
                    result = minimize(
                        frang, c, args=(m2, m1, q1range, j, i), bounds=bnds
                    )
        ...
        if len(triali) >= 1:
            if autoselect:
                if mingrid or uselowest:
                    ...
                    diffofgrid = add(
                        abs(subtract(array(triali), self.numpoints * 2)),
                        abs(subtract(array(trialj), q_other.numpoints * 2)),
                    )
                else:
                    diffofgrid = add(
                        abs(subtract(array(triali), self.numpoints)),
                        abs(subtract(array(trialj), q_other.numpoints)),
                    )
                if self.coordtype == 2:
                    diffofgrid = where(
                        equal(mod(triali, 2), 0), diffofgrid, add(diffofgrid, 1000)
                    )
                if q_other.coordtype == 2:
                    diffofgrid = where(
                        equal(mod(trialj, 2), 0), diffofgrid, add(diffofgrid, 1000)
                    )
                ...
                if self.coordtype == 0 and q_other.coordtype == 0 and not forcegrid:
                    self.maxval, self.minval, q_other.maxval, q_other.minval = vals[
                        itouse
                    ]
                elif self.coordtype == 0 and not forcegrid:
                ...
                if self.coordtype == 0 and q_other.coordtype == 0 and not forcegrid:
                    print(
                        "{0:5} {1:4} {2:4} {3:15} {4:15}".format(
                            "Number", "pts1", "pts2", "q1", "q2"
                        )
                    )
                    for i in range(len(triali)):
                        print(
                            "{0:5} {1:5} {2:5} {4:.3f}-{3:.3f} {6:.3f}-{5:.3f}".format(
                                i,
                                triali[i],
                                trialj[i],
                                vals[i][0],
                    ...
                    itouse = int(
                        input(
                            "choose the number corresponding to the desired number of points: "
                        )
                    )
                    self.maxval, self.minval, q_other.maxval, q_other.minval = vals[
                        itouse
                    ...
                    print(
                        "{0:5} {1:4} {2:4} {3:15} ".format(
                            "Number", "pts1", "pts2", "q1"
                        )
                    )
                    for i in range(len(triali)):
                        print(
                            "{0:5} {1:5} {2:5} {4:.8f}-{3:.8f}".format(
                                i, triali[i], trialj[i], vals[i][0], vals[i][1]
                            )
                        )
                    itouse = int(
                        input(
...
def frr(c, m1, m2, pts1, pts2):
    ...
    from numpy import subtract, power, divide, multiply  # , add, int, abs
...
def frang(c, m1, m2, q2range, pts1, pts2):
    ...
    from numpy import subtract, power, divide, multiply
...
def massweightequal(dq1, m1, dq2, m2, printerrors=False):
    """Function that takes the spacings of coordinates,
    their reduced masses, and returns True/False if they
    are equivalent to 1e-7.
    ...
    if (
        abs(subtract(multiply(power(dq1, 2), m1), multiply(power(dq2, 2), m2)))
        > 1.0e-07
    ):
        if printerrors:
...
def silentmweq(inpcoord=[], mingrid=False, uselowest=False):
    ...
    for x in range(len(inpcoord)):
        coord.append(
            q(
                maxval=inpcoord[x][0],
                minval=inpcoord[x][1],
                coordtype=coordtypedict[inpcoord[x][2]],
                numpoints=inpcoord[x][3],
                mass=inpcoord[x][4],
            )
        )
    ...
    if numcoordinates == 1:
        ...
        if not massweightequal(dq1, coord[0].mass, dq2, coord[1].mass) or mingrid:
            ...
            dq1 = subtract(coord[0].grid[1], coord[0].grid[0])
            dq2 = subtract(coord[1].grid[1], coord[1].grid[0])
            if not massweightequal(
                dq1, coord[0].mass, dq2, coord[1].mass, printerrors=True
            ):
                from sys import exit
    ...
    if numcoordinates == 1:
        ...
        if not massweightequal(dq1, coord[0].mass, dq2, coord[1].mass):
            ...
            if not massweightequal(dq1,
                ...
                exit()
        indices = np.array(
            np.meshgrid(np.arange(coord[0].numpoints), np.arange(coord[1].numpoints))
        ).T.reshape(-1, 2)
        indices = np.split(indices, numcoordinates, axis=1)
        for x in range(len(indices[0])):
            gridout.append(
                str(coord[0].grid[indices[0][x][0]]).ljust(fieldlength)
                + str(coord[1].grid[indices[1][x][0]]).ljust(fieldlength)
            )
    elif numcoordinates == 3:
        indices = np.array(
            np.meshgrid(
                np.arange(coord[0].numpoints),
                np.arange(coord[1].numpoints),
                np.arange(coord[2].numpoints),
            )
        ).T.reshape(-1, 3)
        indices = np.split(indices, numcoordinates, axis=1)
        for x in range(len(indices[0])):
